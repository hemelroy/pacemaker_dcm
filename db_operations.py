...
def connect():
	...
	cur.execute("CREATE TABLE IF NOT EXISTS Attributes (user_id integer NOT NULL, lrLimit, urLimit, vAmplitude, vPWidth,aAmplitude, aPWidth, vrp, arp, avDelay, aThreshold, reactionTime, recoveryTime, msr, FOREIGN KEY(user_id) REFERENCES Users (user_id))")
	conn.commit()
	conn.close()
...
def add_attribute(id,lrLimit, urLimit, vAmplitude, vPWidth,aAmplitude, aPWidth, vrp, arp, avDelay, aThreshold, reactionTime, recoveryTime, msr):
	conn=sqlite3.connect("records.db")
	cur=conn.cursor()
	cur.execute("INSERT INTO Attributes VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",(id, lrLimit, urLimit, vAmplitude, vPWidth,aAmplitude, aPWidth, vrp, arp,avDelay, aThreshold, reactionTime, recoveryTime, msr))
	conn.commit()
	conn.close()
...
def update_attribute(id, parameters):
	conn=sqlite3.connect('records.db')
	cur=conn.cursor()
	cur.execute("Update Attributes set lrLimit=?, urLimit=?,vAmplitude=?,vPWidth=?, aAmplitude=?, aPWidth=?, vrp=?, arp=?, avDelay=?, aThreshold=?, reactionTime=?, recoveryTime=?, msr=? where user_id = ?",(parameters[0],parameters[1],parameters[2],parameters[3],parameters[4],parameters[5], parameters[6], parameters[7], parameters[8], parameters[9], parameters[10], parameters[11], parameters[12], id))
	conn.commit()
	conn.close()
